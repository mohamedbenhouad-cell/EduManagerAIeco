...
def calculate_coherence_score(sentences, tokenizer, model):
    """
    Calcule la cohérence comme la moyenne des probabilités NSP (is_next) pour toutes les paires de phrases consécutives.
    Le score est continu entre 0 et 1.
    """
    if len(sentences) < 2:
        return 1.0  # Un texte d'une seule phrase est considéré comme cohérent
    ...
    for sent_a, sent_b in zip(sentences[:-1], sentences[1:]):
        ...
        logits = outputs.logits
        probs = torch.softmax(logits, dim=1)
        nsp_score = probs[0][0].item()  # Probabilité NSP "is_next"
        nsp_scores.append(nsp_score)
    return float(np.mean(nsp_scores)) if nsp_scores else 1.0
...
def evaluate_question(prof_content, etudiant_content, use_model, nsp_tokenizer, nsp_model, zero_shot, concepts, auteurs, points):
    ...
    coherence_score = calculate_coherence_score(sentences, nsp_tokenizer, nsp_model)
