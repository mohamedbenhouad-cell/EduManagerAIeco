...
import streamlit as st  # <-- Import Streamlit directement
import firebase_admin
from firebase_admin import credentials, firestore

# Initialise Firebase une seule fois (adapter le chemin vers ton fichier de clés)
if not firebase_admin._apps:
    cred = credentials.Certificate("/Volumes/D/my_streamlit_app6/credentials/gestion-budget-personnel-b776e-firebase-adminsdk-fbsvc-0e44a64478.json")
...
def calculate_mean_similarity(text1, text2, use_model):
    ...
    """
    # Convertit les textes en embeddings (attention à la taille des textes, si trop longs, à découper)
...
def load_all_models():
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        nsp_tokenizer = fut_nsp_tokenizer.result()
        nsp_model = fut_nsp_model.result()
        zero_shot = fut_zero_shot.result()
        return use_model, nsp_tokenizer, nsp_model, zero_shot
...
def evaluate_coherence(sentences, tokenizer, model, threshold=1):
    coherent_count = 0
    for sent_a, sent_b in zip(sentences[:-1], sentences[1:]):
        input_ids = tokenizer.encode(sent_a, sent_b, return_tensors='pt')
        ...
        if score >= threshold:
            coherent_count += 1
    return coherent_count

def calculate_coherence_score_with_use(sentences, use_model):
    """
    Calcule la cohérence locale comme la similarité cosinus moyenne
    entre paires de phrases consécutives avec Universal Sentence Encoder.
    Le score est continu entre 0 et 1.
    """
    if len(sentences) < 2:
        return 1.0  # Texte trop court, cohérent par défaut
    pairs = [(sentences[i], sentences[i+1]) for i in range(len(sentences)-1)]
    scores = []
    for sent_a, sent_b in pairs:
        try:
            embeddings = use_model([sent_a, sent_b])
            emb1 = embeddings[0].numpy()
            emb2 = embeddings[1].numpy()
            sim = np.dot(emb1, emb2) / (np.linalg.norm(emb1) * np.linalg.norm(emb2))
            # On force le score entre 0 et 1 (peut parfois être très légèrement hors borne)
            sim = max(0.0, min(sim, 1.0))
            scores.append(sim)
        except Exception as e:
            # Si un problème avec USE, ignore la paire
            continue
...
def evaluate_question(prof_content, etudiant_content, use_model, nsp_tokenizer, nsp_model, zero_shot, concepts, auteurs, points):
    ...
    coherence_score = calculate_coherence_score_with_use(sentences, use_model)
    st.info(f"Score de cohérence : {coherence_score:.2f}")
...
def enregistrer_note_firebase(email, nom_fichier, note):
    """
    Enregistre la note dans la collection "notes_etudiants" pour l'utilisateur identifié par son email.
    Chaque champ du document correspond à un fichier et à la note obtenue.
    """
    doc_ref = db.collection("notes_etudiants").document(email)
    doc_ref.set({nom_fichier: note}, merge=True)
def supprimer_fichiers_dossier (chemin_dossier):
    """
    ...
    try:
        ...
        st.warning(f"Erreur lors de la suppression des fichiers du dossier {chemin_dossier}: {e}")
...
def supprimer_fichiers_dossier(chemin_dossier):
    ...
    try:
        for filename in os.listdir(chemin_dossier):
            ...
            if os.path.isfile(file_path):
                os.remove(file_path)
        # os.rmdir(chemin_dossier)  # décommente pour supprimer aussi le dossier vide
    except Exception as e:
        st.warning(f"Erreur lors de la suppression des fichiers du dossier {chemin_dossier}: {e}")
...
def main(user_email, selected_file):
    ...
    # Enregistrer la note dans Firebase sous le nom du fichier sélectionné
    enregistrer_note_firebase(user_email, selected_file, final_grade)
    ...
    if final_grade > 16:
        st.success("Félicitations pour votre excellente note ! Voici le corrigé du professeur :")
        if os.path.exists(corrige_path):
            # Affichage du PDF dans la page Streamlit (plus de bouton)
            with open(corrige_path, "rb") as f:
    ...
    # Suppression automatique des fichiers du dossier "ben" après affichage
    supprimer_fichiers_dossier("ben")
